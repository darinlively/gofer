...
class TargetNotFound(Exception):
    ...
    def __init__(self, name, exception):
        ...
        super(TargetNotFound, self).__init__(details)
...
class MethodNotFound(Exception):
    ...
    def __init__(self, name):
        ...
        super(MethodNotFound, self).__init__(details)
...
class LogHandler(logutil.LogHandler):
    ...
    def install():
        handler = LogHandler()
        handler.setFormatter(logutil.FORMATTER)
        root = getLogger()
        root.setLevel(INFO)
        root.addHandler(handler)
        LogHandler.installed = handler

    def emit(self, record):
        for r in LogHandler.installed.split(record):
            _log = protocol.Log(r.__dict__)
            _log.send(stdout)
