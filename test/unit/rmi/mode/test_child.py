...
from gofer.agent.logutil import FORMATTER, INFO
from gofer.agent.rmi import Context, Cancelled
from gofer.rmi.mode.child import TargetNotFound, MethodNotFound
from gofer.rmi.mode.child import Progress, Raised, LogHandler, Request, main
...
class TestLogHandler(TestCase):
    ...
    @patch(MODULE + '.getLogger')
    @patch(MODULE + '.LogHandler')
    def test_install(self, handler, get_logger):
        LogHandler.install()
        handler.return_value.setFormatter.assert_called_once_with(FORMATTER)
        get_logger.assert_called_once_with()
        get_logger.return_value.setLevel.assert_called_once_with(INFO)
        get_logger.return_value.addHandler.assert_called_once_with(handler.return_value)
        self.assertEqual(handler.installed, handler.return_value)

    @patch(MODULE + '.LogHandler.installed')
    def test_emit(self, installed):
        stdout = StringIO()
        record = Mock()
        record.__dict__ = {'A': 1}
        installed.split.return_value = [record]
        ...
        stdout.seek(0)
        reply = protocol.Reply.read(stdout)
        self.assertEqual(reply.code, protocol.Log.CODE)
        self.assertEqual(reply.payload, record.__dict__)
