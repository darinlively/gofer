...
from gofer.rmi.mode.parent import Monitor, Result, Progress, Error, Raised, Log, Request
...
class TestReplies(TestCase):
    ...
    @patch(MODULE + '.new')
    @patch(MODULE + '.LogHandler.find')
    def test_log(self, find, new):
        payload = {'A': 1}
        reply = Log(payload)
        ...
        # validation
        new.assert_called_once_with(LogRecord, payload)
        find.return_value.emit.assert_called_once_with(new.return_value)
